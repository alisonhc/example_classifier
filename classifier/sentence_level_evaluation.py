import jsonlines
from classifier.utils import MULTI_LABEL_TO_INDEX
...
def get_eval_metrics(result_fpath, labels_fpath):
    ...
    for i in range(len(result_probs)):
        pred_class = result_probs[i].index(max(result_probs[i]))
    ...
    print(f"Class acc: {class_acc}\nGeneral Level Acc: {general_level_acc}")
    print(f"Percentage of predictions 1 or less deviation from actual level: {less_than_one_deviation_percent}")
